...
**Note**: This repo includes the initial version of Piano. This version is now described as a variant in the online version of our paper (Appendix A).
...
2. In another terminal, `go run client/client.go -ip localhost:50051 -thread 1`. This runs the PIR experiment with one setup phase for a window of $\sqrt{n}\ln(n)$-queries and follows with the online phase of up to 1000 queries. The ip flag denotes the server's adddress. The thread denotes how many threads are used in the setup phase.
